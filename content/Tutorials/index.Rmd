...
First, we specify the same model in `JAGS` in a text file:
...
Now, we estimate the model again and ask it to monitor all the sampled random effects, which we will treat as multiple imputed values. We run the sampler by using two chains with a 1000 iterations each (excluding the burn-in period of a 1000 extra iterations).
...
Extract the random $\alpha_{j}s$ and $\beta_{j}s$s into separate data frames: 
...
Extract these in a big matrix with the samples from every iteration stored as a separate column:
...
Extract them in a big matrix with the samples from every iteration stored as a separate column:
...
We calculate the BFs by using the ICC-based $N_{eff}$ = 358 (which can be calculated automatically within the `wrapper function`) since the MI-based $N_{eff}$ has not yet been extended to include level-2 predictors and, as shown in the paper, the sample size does not influence the BF when using $J_{ref}$ and the effect size in our data is large.
...
```
 ...
 Additionally, $BF_{u0} \simeq 1.9\text{e+}167$ and $BF_{iu}\simeq 2.4\text{e+}170$.
...
# Example 5: Model that includes a level-2 predictor and a cross-level interaction

Finally, using the data set with the simulated outcome with no effect of the predictors, we estimate a model that includes a cross-level interaction between `standlrt` and `avslrt`. 
...
As can be seen from Table 3, based on its standard error, the cross-level interaction effect is significant, even though the direct fixed effects of both level-1 and level-2 predictors aren't. 

```{r}
hypotheses <- "standlrt = avslrt = standlrt:avslrt = 0;
...
When applying the proposed framework on this model, we obtain  $BF_{0u} = 2.1$ and a $BF_{iu} = 1$, which translates to $BF_{0i} \approx 2$. This suggests that when including an interaction effect, $BF_{0u}$ and consequently $BF_{0i}$ are smaller relative to their counterparts from Example 4, which indicates that the interaction effect is larger than zero. The *fraction b* is now 0.14, which means that this should be explicitly stated when reporting the results. More specifically, it should be stated that the values given by `bain`, in this case are treated as an information criteria that are inspired by the BF.
