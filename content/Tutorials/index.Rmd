...
This tutorial represents a complete and extensive version of the *Example* section from my master's thesis. 
...
- `hypotheses`: the specified hypothesis/hypotheses (saved as a character vector); 
...
$$R^2_m = \frac{\sigma_f}{\sigma_f + \sigma_{u0} + \sigma_{u1} + \sigma_\epsilon}.$$
```{r}
...
$$Z_{ij} = \text{normexam}_{ij} - \alpha_j - \beta_j*\text{standlrt}_i + \alpha + \beta*\text{standlrt}_i$$
